# Artiom Petkun — Project Portfolio

Hi! I'm Artiom, a junior developer from Vilnius, Lithuania — Game Development graduate (Vilnius Business College, 2025). This repository collects my personal projects. Each folder is a self-contained project with its own docs and build.

## 📂 Projects

| Project | Tech | Description |
|---|---|---|
| [Clicker Game](#-clicker-game) | Unity, C# | [One-line pitch: what the player does and why it's fun] |
| Home Lab | Raspberry Pi, Linux, DNS/DHCP, SSH | My home network lab — Pi-hole ad-blocking, remote administration *(separate repo coming soon)* |

---

## 🎮 Clicker Game

**Engine:** Unity `[VERSION, e.g. 2022.3 LTS]` · **Language:** C# · **Status:** Playable build available

An incremental clicker where the player grows [X] from a single click to an automated empire. Built to practise core Unity workflows: UI, game state, prestige mechanic.

### ✨ Features

- Click-to-earn core gameplay loop
- Upgrade shop with scaling prices
- Passive / auto-income generators
- Prestige / reset mechanic 

### ▶️ Play it

- **Windows build:** see the `Build` folder inside `C#/Unity/Clicker game`

### 🛠 Run from source

1. Clone this repository
2. In Unity Hub: **Add → Add project from disk** → select `C#/Unity/Clicker game`
3. Open with Unity `[VERSION]`
4. Open the main scene (`Assets/Scenes/Main.unity`) and press **Play**

### 🧠 What I learned building this

- Unity UI system and event handling
- Balancing an economy curve
- [e.g. version control with Git and large binary assets]

---

## 📬 Contact

- **Email:** mrgrom28@gmail.com
- **Location:** Vilnius, Lithuania
